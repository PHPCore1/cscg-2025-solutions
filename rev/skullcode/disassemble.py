...
class Disassembler:
		...
		if instr & 0x8000: #Immediate
			...
			if type == 0: return "0x%02X" %(instr >> 24)
				...
				if register == 0: return "0x%08X" %self.fetch()
					...
					return "E_INVALID"
			elif type == 3: return "0x%08X%08X" % tuple(reversed((self.fetch(), self.fetch())))
			elif type in [4, 7]: return "0x%08X%08X%08X%08X" % tuple(reversed((self.fetch(), self.fetch(), self.fetch(), self.fetch())))
	...
	def opcode_mov(self, instr):
		...
		if src.startswith('0x') and int(src[2:], 16) in self.symbols:
			src = self.symbols[int(src[2:], 16)]
	...
	def disassemble(self, data, outfile, symbols):
		...
		while self.pc < len(data):
			addr = self.pc
			visual_addr = addr + 0x4400

			if visual_addr in symbols:
				self.write("%s:" %symbols[visual_addr])

			instr = self.fetch()
			opcode = instr & 0xFF
			if opcode in self.opcodes:
				self.write("%04X:%08X: %s" %(visual_addr, instr, self.opcodes[opcode](instr)))
			else:
				self.write("%04X:%08X: invalid opcode %i" %(visual_addr, instr, opcode))
